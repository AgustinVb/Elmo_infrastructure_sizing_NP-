...
\noindent \change{No se impone condición de borde diaria explícita sobre $S$ por sí sola (solo sobre $X^{dch}$, \eqref{eq:borde_dch}): la identidad de inventario total \eqref{eq:inventario_total}, evaluada en $t_{ini}$ para todo día representativo, junto con \eqref{eq:borde_dch}, ya implica el cierre cíclico de $S$ en $t_{ini}$. $Sv_{k,y,d,t,a}$ solo existe como variable para $a$ dentro de la ventana válida $\mathcal{A}(t)$ (Sección \ref{sec:estaciones}); \eqref{eq:def_sv}/\eqref{eq:def_sv_carry} se aplican solo dentro de esa ventana.}
