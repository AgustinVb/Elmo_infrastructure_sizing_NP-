"""
Exporta, para cada escenario que usa descomposicion por macrobloques (nombre
contiene "Descomp"), un CSV con el material total extraido (variable M) por
punto de extraccion (nodo), agregando LHDs e intervalos de un dia
representativo (la extraccion es la misma en todos los dias/meses
representativos, asi que basta con uno).

Reusa:
  - combine_macrobloques_for_plot.py: para generar combined/day_X/ en los
    escenarios que aun no lo tienen (p.ej. Ventana_restringida).
...
def find_descomp_scenarios(root: Path) -> list[Path]:
    """Directorios cuyo nombre contiene "descomp" (case-insensitive), en
    cualquier profundidad bajo root."""
    return sorted(
        d for d in root.rglob("*")
        if d.is_dir() and "descomp" in d.name.lower()
    )
...
def main():
    ...
    print(f"Escenarios _Descomp encontrados: {len(scenarios)}")
    for s in scenarios:
    ...
    for scenario in scenarios:
        ...
        try:
            ...
            # sep=";" y decimal="," para que Excel en configuracion regional
            # español (donde "." es separador de miles) lo lea correctamente.
            extr.to_csv(out_path, index=False, sep=";", decimal=",")

            print(f"  dia representativo: {day_dir.name}")
            print(f"  nodos: {len(extr)}  |  total extraido: {extr['total_extracted_tons'].sum():.1f} t")
            print(f"  ✅ {out_path}")
            ok += 1
