...
def load_year_indexed_df(path: str, value_name: str = "value") -> Optional[pd.DataFrame]:
    """Carga un JSON de una variable indexada solo por año (ej. R, b_bar, N_ciclos,
    CumEFC) → DataFrame con columnas [year, <value_name>].
    Tolera tanto eje nombrado "y" como el genérico "_1" (Printer usa "_1" para
    variables que no están en var_index_order de opt_model.py)."""
    data = _load_json(path)
    if not data:
        return None
    rows = []
    for tokens, val in _collect_named_leaf_records(data):
        axes = {n: v for n, v in tokens}
        if len(axes) == 1:
            year_val = next(iter(axes.values()))
            rows.append({"year": int(year_val), value_name: float(val)})
...
class Parameters:
    ...
    def _load(self):
        ...
        # (time_intervals_peak_set). Vacia en corridas que no la exportan ->
        # el plotter cae al calculo local por punto medio.
...
class JSONPlotter:
    ...
    def _det_or_legacy_shade_specs(self):
        ...
        if det_intervals_available:
            ...
            peak_intervals = (list(getattr(self.params, "peak", []) or [])
                              or self._build_intervals_from_clock_windows(peak_windows, start_hour=9))
        else:
