...
WP2, los 6 escenarios de swap de data/Escenarios_DCH_agosto (Costo_fijo +
Costo_variable) en las dos ventanas de swap DCH: restringida (solo
colacion/entre-turnos) y libre (cualquier momento salvo mantencion). En
total 6 x 2 = 12 corridas, cada una de 36 subproblemas (3 estaciones x 12
dias x 2 fases).

Portado desde run_batch_dch_agosto.py (rama carga_on_board), adaptado a
terminologia de swap: --pause_scheme dch + --swap_window {restringida,libre}
en vez de --charge_window.

Requisitos verificados antes de escribir este script:
- Los 6 escenarios ya tienen electric_routes_within_time.json y base_hour=9
  en la hoja Shifts de time_series.xlsx.
- --pause_scheme dch registra maintenance_stop_all + maint_no_swap +
  meal_g1/g2_no_travel (en vez de det_stop_all); --swap_window restringida
  ademas agrega swap_only_meal_or_between_shifts. Verificado con un smoke
  test que compara los nombres de restricciones registradas bajo cada
  combinacion de flags.

Salida: output/DCH_agosto/{Costo_fijo,Costo_variable}/{Swap_restringido,Swap_libre}/<escenario>_Descomp/

Uso:
    python run_batch_swap_agosto.py
    python run_batch_swap_agosto.py --dry_run
    python run_batch_swap_agosto.py --only Costo_fijo
    python run_batch_swap_agosto.py --only 320kW,530kW
    python run_batch_swap_agosto.py --windows restringida
    python run_batch_swap_agosto.py --skip_existing false
...
--parallel_days internamente con varios workers (y un lock compartido que
serializa la construccion de Series() entre ellos, ver
_build_series_with_retry en run_descomposicion.py). Si una combinacion falla
...
registra el error y se sigue con la siguiente.
"""
import argparse
import os
import re
...
def discover_scenarios(only=None):
    """Enumera (cost_dir, scenario_path) para los escenarios de swap,
    opcionalmente filtrados por --only (nombre de cost_dir o substring del
    nombre de escenario)."""
    scenarios = []
...
def main():
    parser = argparse.ArgumentParser(
        description="Corre con descomposicion + WP2, en ambas ventanas de swap DCH, "
                     "todos los escenarios de swap de data/Escenarios_DCH_agosto."
    )
    parser.add_argument(
        "--only", default=None,
    ...
    parser.add_argument(
        ...
        help="MIPGap objetivo para Gurobi, pasado a run_descomposicion.py --gap (default: 0.01 = 1%%)."
    )
    parser.add_argument(
        "--timelimit", type=int, default=3600,
        help="Timelimit en segundos por subproblema, pasado a run_descomposicion.py --timelimit (default: 3600)."
    )
    parser.add_argument(
        "--dry_run", action="store_true",
        help="Solo imprime los 12 (o menos, segun filtros) comandos que se ejecutarian, sin correr nada."
    )
    ...
    print("=" * 100)
    print(f"{'cost_dir':14s} {'escenario':40s} {'ventana':12s} {'estado':14s} {'costo_total':>15s} {'ok':>4s} {'infact.':>8s}")
    for cost_dir, name, window, status, total_cost, n_ok, n_infeasible in results:
        ...
        print(f"{cost_dir:14s} {name:40s} {window:12s} {status:14s} {cost_str:>15s} "
