"""
Descompone el problema completo (battery swapping, N estaciones) en subproblemas
independientes, uno por macrobloque/estacion de swap, leyendo UNA SOLA VEZ los
datos completos (elmo_data.xlsx + time_series.xlsx) y filtrando en memoria por
estacion. Generalizacion de carga_on_board/run_descomposicion.py para BS.
...
Modo normal (--days con 1+ dias, sin --parallel_days):
    ...
    usando solo su propia infraestructura de swap como cota.

Modo --parallel_days (descomposicion adicional por dia, en paralelo):
    A diferencia de carga_on_board, en BS el ciclo cerrado de bateria por dia
    (B[i,d,0] == B[i,d,tf], vease battery_energy_conservation en functions.py)
    ya es una restriccion nativa del modelo para CUALQUIER corrida, no algo que
    haya que agregar especialmente para la decomposicion: los dias en BS ya son
    independientes entre si en SOC salvo por:
      - infraestructura de swap (N_chargers, N_bays, N_batteries, X): se decide
        UNA vez para todo el horizonte, no por dia.
      - P_pot/p_peak: acoples ya discutidos para el caso por estacion.
    Por eso se resuelve en dos sub-fases, igual que en carga_on_board:
...
Generators/Storage hasta resolver eso explicitamente. Misma limitacion que
carga_on_board.
...
# Variables de infraestructura de swap que se deciden una vez para todo el
# horizonte y se agregan (maximo entre dias) al pasar de Fase 1 a Fase 2.
# Deben coincidir con las claves que entiende OptModel._apply_fixed_infra.
INFRA_VARS = ('N_chargers', 'N_bays', 'N_batteries', 'X')
...
def solve_macrobloque_day(job):
    ...
    for var_name in INFRA_VARS:
        result[var_name] = _read_station_scalar(output_folder, var_name, station_name)
    return result
...
def main():
    parser = argparse.ArgumentParser(description='Descomposicion por macrobloque/estacion (y opcionalmente por dia, en paralelo) para battery swapping')
