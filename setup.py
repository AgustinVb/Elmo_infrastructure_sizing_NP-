...
def resolve_wp2_json_path(args, mine_system):
    """
    Resuelve la ruta del JSON de consumos WP2.

    Si no se entrega `--wp2_consumption_json`, se infiere segun la
    tecnologia de la flota: todo diesel -> diesel_routes_within_time.json,
    sin diesel -> electric_routes_within_time.json. Una flota mixta exige
    que el archivo se pase de forma explicita. La ruta se resuelve relativa
    a `data_folder` salvo que ya sea absoluta.
    """
