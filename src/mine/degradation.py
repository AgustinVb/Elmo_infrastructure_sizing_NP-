...
class BatteryDegradation(object):
    """Datos globales de degradacion/reemplazo de baterias del pool de swap.
    ...
    def get_c_bat_replace(self) -> float:
        """Costo total de reemplazar UNA bateria del pool de swap [USD]. Se
        escala por la cantidad total de baterias del pool al calcular el
        costo de reemplazar el pool completo (ver
        ObjectiveRules.battery_replace_cost)."""
