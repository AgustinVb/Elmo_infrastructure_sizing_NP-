...
class Timeseries(object):
    ...
    def _load_wp2_consumption_by_node(self, json_path, required_nodes) -> dict:
        """
        Carga y valida el JSON de consumos WP2 (por nodo, precalculado).

        Estructura esperada de primer nivel: {node_name: {..., 'time_per_cycle_s': x,
        'energy_per_cycle_kwh': y, ...}}.

        :param json_path: ruta al archivo JSON.
        :param required_nodes: iterable de nombres de nodo que deben estar presentes.
        ...
        if not os.path.isfile(json_path):
            raise FileNotFoundError(f"No se encontro el JSON de consumos WP2 en: {json_path}")
        ...
        if not isinstance(raw, dict):
            raise ValueError(
                f"El JSON de consumos WP2 ({json_path}) debe ser un objeto de primer nivel "
                "indexado por nombre de nodo."
            )
