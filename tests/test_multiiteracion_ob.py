...
from test_gate_cortes_ob import _cache_path
...
def main(scenario, max_iter, n_years):
    ...
    for h in res["gap_history"]:
        ...
        resumen = {}
        for fam, val in sorted(x.items()):
            resumen[fam] = ({k2: round(v2, 3) for k2, v2 in val.items()}
                            if isinstance(val, dict) else round(val, 3))
        print(f"  k={h['iteration']:<3} {resumen}", flush=True)
